...
void ignis_display_init(void)
{
    ...
    {
        const tm1637_config_t config = {
            .clk_pin = IGNIS_DISPLAY_PIN_CLK, .dio_pin = IGNIS_DISPLAY_PIN_DATA, .bit_delay_us = 100
        };
...
void ignis_display_send_numeric_data(const uint8_t* data, const bool time)
{
    ...
    char buffer[6] = {0};
    snprintf(buffer, sizeof(buffer), "%c%c%c%c", ignis_display_ctx.display_digits[0],
             ignis_display_ctx.display_digits[1], ignis_display_ctx.display_digits[2],
